...
Deduplication is the removal of duplicate block sizes from a dataset. For example, if the same 4K block existed in 10 different files, deduplication would redirect that 4K block within all 10 files to the same 4K physical block. The result would be a 10:1 improvment in efficiency for that data.
...
* If volumes and/or LUNs are not thin provisioned, you should must disable all efficiency settings because using these features provides no savings and the combination of thick provisioing with space efficiency enabled can cause unexpected behavior, including out-of-space errors. 
* If data is not subjuct to overwrites, such as with backups or database transaction logs, you can achieve greater efficiency by enabling TSSE with a low cooling period. 
