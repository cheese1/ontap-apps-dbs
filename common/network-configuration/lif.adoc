...
* *SVM management LIFs.* Interfaces that permit access to an SVM through the ONTAP API (known as ZAPI) for functions such as snapshot creation or volume resizing. Products such as SMO must have access to an SVM management LIF.
