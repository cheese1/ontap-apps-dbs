...
keywords: data protection, integrity, checksums, corruption, RAID
summary: Data integrity features with ONTAP
---

= Data integrity with ONTAP
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
To protect data from this situation, ONTAP allows volumes to be configured for special protection against NVRAM failure. When triggered, this protection mechanism results in a volume entering a state called NVFAIL. This state results in I/O errors that cause a an application shutdown so that they do not use stale data.  Data should not be lost because any acknoowledged write should be present on the storage array.
