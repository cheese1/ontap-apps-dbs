---
sidebar: sidebar
permalink: oracle/migration/procedures-host_data_copy.html
keywords: TR-4534, migration, oracle, data copy, lvm, rebalance, asm
summary: Oracle migration using the host-side storage stack
---

= Procedures - host data copy
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
At the simplest level, data can be moved with a simple copy operation.
...
+
....
ontap-nfs1:/jfs0_oradata       52428800  16196928  36231872  31% /oradata
ontap-nfs1:/jfs0_logs          49807360    548032  49259328   2% /logs
....

* New file systems
+
....
ontap-nfs1:/jfs0_logs_new      49807360       128  49807232   1% /new/logs
ontap-nfs1:/jfs0_oradata_new   49807360       128  49807232   1% /new/oradata
....
...
+
....
[root@jfs0 current]# ./dbshut.pl NTAP
...
. Convert the file systems to read-only. This can be done more quickly by using a script, as shown in link:sample_scripts.html#convert-file-system-to-read-only[Convert File System to Read Only].
+
....
[root@jfs0 current]# ./mk.fs.readonly.pl /oradata
/oradata unmounted
/oradata mounted read-only
[root@jfs0 current]# ./mk.fs.readonly.pl /logs
/logs unmounted
...
+
....
ontap-nfs1:/jfs0_oradata on /oradata type nfs (ro,bg,vers=3,rsize=65536,wsize=65536,addr=172.20.101.10)
ontap-nfs1:/jfs0_logs on /logs type nfs (ro,bg,vers=3,rsize=65536,wsize=65536,addr=172.20.101.10)
....

. Synchronize file system contents with the `rsync` command.
+
....
[root@jfs0 current]# rsync -rlpogt --stats --progress --exclude=.snapshot /oradata/ /new/oradata/
...
[root@jfs0 current]# rsync -rlpogt --stats --progress --exclude=.snapshot /logs/ /new/logs/
sending incremental file list
./
NTAP/
...
. Unmount the old file systems and relocate the copied data. This can be done more quickly by using a script, as shown in link:sample_scripts.html#replace-file-system[Replace File System].
+
....
[root@jfs0 current]# ./swap.fs.pl /logs,/new/logs
/new/logs unmounted
/logs unmounted
Updated /logs mounted
[root@jfs0 current]# ./swap.fs.pl /oradata,/new/oradata
...
+
....
ontap-nfs1:/jfs0_logs_new on /logs type nfs (rw,bg,vers=3,rsize=65536,wsize=65536,addr=172.20.101.10)
ontap-nfs1:/jfs0_oradata_new on /oradata type nfs (rw,bg,vers=3,rsize=65536,wsize=65536,addr=172.20.101.10)
....

. Start the database.
+
....
[root@jfs0 current]# ./dbstart.pl NTAP
ORACLE_SID = [oracle] ? The Oracle base has been set to /orabin
...
....
[root@jfs0 current]# ./migrate.oracle.fs.pl NTAP /logs,/new/logs /oradata,/new/oradata
....
...
+
....
[root@jfs0 current]# ./checkdbdata.pl TOAST
TOAST datafiles:
...
+
....
[root@jfs0 ~]# pvdisplay -C | grep datavg
...
+
....
[root@jfs0 ~]# pvcreate /dev/mapper/3600a098038303537762b47594c315864
  Physical volume "/dev/mapper/3600a098038303537762b47594c315864" successfully created
[root@jfs0 ~]# pvcreate /dev/mapper/3600a098038303537762b47594c315863
  Physical volume "/dev/mapper/3600a098038303537762b47594c315863" successfully created
[root@jfs0 ~]# pvcreate /dev/mapper/3600a098038303537762b47594c315862
  Physical volume "/dev/mapper/3600a098038303537762b47594c315862" successfully created
[root@jfs0 ~]# pvcreate /dev/mapper/3600a098038303537762b47594c315861
...
+
....
[root@jfs0 tmp]# vgextend datavg /dev/mapper/3600a098038303537762b47594c315864
  Volume group "datavg" successfully extended
[root@jfs0 tmp]# vgextend datavg /dev/mapper/3600a098038303537762b47594c315863
  Volume group "datavg" successfully extended
[root@jfs0 tmp]# vgextend datavg /dev/mapper/3600a098038303537762b47594c315862
  Volume group "datavg" successfully extended
[root@jfs0 tmp]# vgextend datavg /dev/mapper/3600a098038303537762b47594c315861
  Volume group "datavg" successfully extended
....
...
+
....
[root@jfs0 tmp]# pvmove -i 10 /dev/mapper/3600a098038303537762b47594c31582f /dev/mapper/3600a098038303537762b47594c315864
...
[root@jfs0 tmp]# pvmove -i 10 /dev/mapper/3600a098038303537762b47594c31585a /dev/mapper/3600a098038303537762b47594c315863
...
[root@jfs0 tmp]# pvmove -i 10 /dev/mapper/3600a098038303537762b47594c315859 /dev/mapper/3600a098038303537762b47594c315862
...
[root@jfs0 tmp]# pvmove -i 10 /dev/mapper/3600a098038303537762b47594c31586c /dev/mapper/3600a098038303537762b47594c315861
...
+
....
[root@jfs0 tmp]# vgreduce datavg /dev/mapper/3600a098038303537762b47594c31582f
Removed "/dev/mapper/3600a098038303537762b47594c31582f" from volume group "datavg"
[root@jfs0 tmp]# vgreduce datavg /dev/mapper/3600a098038303537762b47594c31585a
  Removed "/dev/mapper/3600a098038303537762b47594c31585a" from volume group "datavg"
[root@jfs0 tmp]# vgreduce datavg /dev/mapper/3600a098038303537762b47594c315859
  Removed "/dev/mapper/3600a098038303537762b47594c315859" from volume group "datavg"
[root@jfs0 tmp]# vgreduce datavg /dev/mapper/3600a098038303537762b47594c31586c
