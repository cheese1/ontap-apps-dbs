---
sidebar: sidebar
permalink: hana/fas-fc/host_setup.html
...
[.lead]
Before setting up the host, NetApp SAN Host Utilities must be downloaded from the http://mysupport.netapp.com/[NetApp Support^] site and installed on the HANA servers. 

The Host Utility documentation includes information about additional software that must be installed depending on the FCP HBA used.
...
stlrx300s8-6:/ # chmod â€“R 777 /hana/log/SS3
stlrx300s8-6:/ # chmod â€“R 777 /hana/data/SS3
...
. To mount the file systems, run the `mount â€“a` command at each host.

link:i_o_stack_configuration_for_sap_hana.html[Next: I/O stack configuration for SAP HANA.]
