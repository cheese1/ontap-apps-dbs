---
sidebar: sidebar
permalink: hana/fas-nfs/adding_additional_data_volume_partitions.html
...
[.lead]
Starting with SAP HANA 2.0 SPS4, you can configure additional data volume partitions, which allows you to configure two or more volumes for the data volume of an SAP HANA tenant database.

You can also scale beyond the size and performance limits of a single volume.
...
In addition to the tasks described in the section link:host_setup.html[“Host setup,”] you must create the additional mount points and fstab entries for the new additional data volume(s), and you must mount the new volumes.
