---
sidebar: sidebar
permalink: vmware/vsphere/other_capabilities_for_vsphere.html
...
* snapshots might lock blocks that could be reduced by compression or deduplication. When using scheduled background efficiency or one-time scanners, make sure that they run and complete before the next snapshot is taken. Review your snapshots and retention to make sure you only retain needed snapshots, especially before a background or scanner job is run.
