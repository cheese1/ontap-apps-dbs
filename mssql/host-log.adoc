...
* Make sure that the user database and the host log directory are on separate volumes to prevent the retention policy from overwriting snapshots when these are used with SnapVault technology.
